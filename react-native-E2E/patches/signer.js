...
export const generate = () => {
  const key = nacl.sign.keyPair().secretKey;
  return derive(key);
  //   derive(ED25519.utils.randomPrivateKey());
};
...
class SigningAuthority {
  ...
  sign(payload) {
    ...
    var mergedArray = new Uint8Array(KEY_SIZE * 2);
    mergedArray.set(this.secret);
    mergedArray.set(this.publicKey, KEY_SIZE);
    const signature = nacl.sign.detached(payload, mergedArray);
